...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val result = mapA.mapValues { linkedSetOf(it.value) }.toMutableMap()
    for ((key, value) in mapB) {
        if (key in result)
            result[key]?.add(value)
        else
            result[key] = linkedSetOf(value)
    }
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> =
    stockPrices.groupBy { it.first }.mapValues { it.value.sumByDouble { e -> e.second } / it.value.size }
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? =
    stuff.filter { it.value.first == kind }.entries.minWithOrNull(compareBy { it.value.second })?.key
...
fun extractRepeats(list: List<String>): Map<String, Int> =
    list.groupingBy { it }.eachCount().filter { e -> e.value >= 2 }
...
fun hasAnagrams(words: List<String>): Boolean =
    words.map { it.reversed() }.toSet().intersect(words.toSet()).isNotEmpty()
...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    var prev = friends.toMutableMap()
    val new = prev
    do {
        for (key in new.keys) {
            val newValue: MutableSet<String> = new[key]?.toMutableSet() ?: continue
            for (value in new[key]!!) {
                if (value !in new)
                    new[value] = setOf()
                newValue.addAll(new[value]!!)
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        if ((number - list[i]) in mapIndices) {
            return Pair(mapIndices[number - list[i]]!!, i)
        }
        mapIndices[list[i]] = i
    }
    return Pair(-1, -1)
